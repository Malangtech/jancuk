...
$cekdate = $koneksi_db->sql_query("SELECT judul,waktu_mulai,waktu_akhir,isi,background,color FROM kalender WHERE left( `waktu_mulai` , 7 ) = '$varwaktucalender'");
...
$toolstips_cal_EVENT = "\r\n". $TMPpesan__[$valitemselectedday] ."<br /><br />&raquo; <a href=\"index.php?pilih=kalender&amp;mod=yes&amp;act=calendar_view&amp;action=detail&amp;sel_date=".$sel_date."&amp;waktu_akhir=".$TMPtgl_akhir[$valitemselectedday]."\">Selengkapnya</a>\r\n";       
}
}
/////sekarang
else {
...
$links[0] = '<a title="'.$JUDULCAL[date('j', $i)].'" href="index.php?pilih=kalender&amp;mod=yes&amp;act=calendar_view&amp;sel_date='. $i .'" style="text-decoration:none;'.$color.'">';
$links[1] = '</a>';
$toolstips_cal .= '<div id="calendar_'.$valitemselectedday.'" style="display:none;POSITION: absolute; PADDING-RIGHT: 10px; PADDING-LEFT: 10px; PADDING-BOTTOM: 10px; WIDTH: 160px; PADDING-TOP: 10px; BACKGROUND-COLOR: #efefef;border:1px solid #b1b1b1;font-family:verdana,tahoma,arial;font-size:9px;line-height:13px">
'.$TMPpesan[$valitemselectedday].'</div>';
}else {
$stylebold = '';  
$links[0] = '';
$links[1] = '';  	
